...
var VersusComponent = Vue.component("versus", {
	...
	methods: {
		...
		winnerDetermined: function winnerDetermined() {
			...
			var char1Winners = this.winnerArray1.filter(function (char) {
				return _this.player1Chars.includes(char);
			}).length;
			console.log("Player 1 Winners: ", char1Winners);
			var char2Winners = this.winnerArray1.filter(function (char) {
				return _this.player2Chars.includes(char);
			}).length;
			...
			if (char2Winners === char1Winners) {
				var randomWinner = this.winnerArray1[Math.floor(Math.random() * this.winnerArray1.length)];
				this.winnerArray1 = [randomWinner];

				char2Winners = this.winnerArray1.filter(function (char) {
					return _this.player2Chars.includes(char);
				}).length;
				console.log("Player 1 Winners: ", char1Winners);
				char2Winners = this.winnerArray1.filter(function (char) {
